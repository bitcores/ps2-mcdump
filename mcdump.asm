...
;; if successful, 8MB card can be dumped with ps2client with the following command
;; ./ps2client -h <ps2 ip address> dumpmem 2097152 8650752 dump.txt
;; if an error occurs (usually reads FFFF for size of card) will return to ps2 menu
...
;; gotta make sure the cache pointer list is zeroed each time
	li $s0, mcblkcachep
	sw $zero, ($s0)
...
;; load size of memory card from specs
	...
	and $a0, $a0, $zero
Dump_MC_loop:
	jal Memorycard_findpage
	nop
	addi $a0, $a0, 0x10
	slt $s2, $a0, $s4
	bne $s2, $zero, Dump_MC_loop
	nop
...
Memorycard_cacheblock:
	...
	dadd $s0, $s0, $s2
	dsll $s2, $s1, 8		; multiply index by 512 (crc in block)
	dadd $s0, $s0, $s2
	li $s7, mcbuffer
	ori $s2, $zero, 0x10
...
Show_done_msg:
	addi $sp, $sp, -8
	sd $ra, 0($sp)
	li $a0, donetitle_packet
	li $a1, donetitle_packet_end
	jal GIF_send_data
	nop
	jal vsync_wait
	nop
...
donetitle_packet:
	...
	dc64 GIF_TAG(((donetitle_bin_end - donetitle_bin) / 16), 1, 0, 0, FLG_IMAGE, 1), REG_A_D
donetitle_bin:
	.binfile "DONEstr.img"
donetitle_bin_end:
donetitle_packet_end:
.align 128
	dc64 1
